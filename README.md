...
bundle interact with gem file, ensures all dependecies are available for gem files

resource command looks at name of Model and column info, makes a migration, makes a model class, controller class, spec

remove views because we have an API
$ rm -rf view file

resource gives us the 7 restful routes for the tracker...still need to know them...but don't have to create them

$ rails routes

## Make entries
Student.create name: 'Charlean', cohort: 'Delta'
Student.all
...
Index - shows all data
  def index
    ...
    render json: students  ---> verify that the data will appear in postman
  end
...
Create - posts new data (need strong params, private so info does not get called outside the class, give conditional that shows error message)
  def create
...
Update - modifies data(similar to show and create)
  ...
  def update
    one_stud = Student.find(params[:id])
    one_stud.update(studen_params)
    if one_stud.valid?
      render json: one_stud
    else
      render json: one_stud.errors
    end
  end
...
Interact with controller, hit the index
Get the convention for the route
index controller method--->follow the rails route
...
show controller method--->student variable

http verb ---> copy url from browser 
GET             localhost:3000/students/1

create controller method--->student variable

Go to body tab, raw radio button, json drop down
Create json
id, created_at, updated_at ----> handled by rails. Just update name/cohort
{
  "name": "Ashley"
  "cohort": "Delta"
}
...
http verb ---> copy url from browser 
PATCH             localhost:3000/students/2
...
RAILS DOES NOT DELETE PRIMARY KEYS. It will assign the next value.
